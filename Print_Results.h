...
void Print_Mesh(Dat_Info &D, std::vector<Node> &N, std::vector<Element> &E, std::vector<Element> &EI, std::ofstream& outFile)
{
	outFile << "#==================================================================================================================================\n";
	...
	outFile << "Interface Elements" << std::endl;
	outFile << "#  Ielem  Conectivities  Lmat LcoH LcoM LcoT Ltip Nnom Nnof Ngau  Conectivities " << std::endl;
	outFile << "#------+-------+-------+----+----+----+----+----+----+----+----+-------+-------+" << std::endl;
	for (int i = 0; i < D.NELINT; i++)
	{
		outFile << std::setfill(' ') << std::setw(8) << EI[i].I_element;
		for (int j = 0; j < (int)E[i].Node_Numbers.size()/2; j++)
			outFile << std::setfill(' ') << std::setw(8) << EI[i].Node_Numbers[j];
		outFile << std::setfill(' ') << std::setw(5) << EI[i].Lmat;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].LcoH;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].LcoM;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].LcoT;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].Ltip;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].Nnom;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].Nnof;
		outFile << std::setfill(' ') << std::setw(5) << EI[i].Ngau;
		for (int j = (int)E[i].Node_Numbers.size() / 2; j < (int)EI[i].Node_Numbers.size(); j++)
			outFile << std::setfill(' ') << std::setw(8) << EI[i].Node_Numbers[j];
		outFile << std::endl;
	}
...
void Print_Results(Dat_Info &D, std::vector<Node> &N, std::vector<Element> &E, std::vector<Element> &EI, std::vector<Material> &M, std::string Address)
{	
	std::ofstream File;
	File.open(Address);
